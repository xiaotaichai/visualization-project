...
library(dslabs)
library(plotly)
knitr::opts_chunk$set(message = FALSE, warning = FALSE, fig.show = "animate", out.width = 1000, out.height = 600)
...
diseases.location <- diseases %>%
  mutate(rate = (count / weeks_reporting) * 52 / (population / 10000))
p3 <- ggplot(diseases.location, aes(state, rate, color = disease, frame = year)) +
  geom_point(size = 3)+
...
gganimate(p3, ani.width=1000, ani.height=600)
```
